...
public class RobotSimulationService {
    ...
    public RobotSimulationService(RobotRepository robotRepo, RobotActivityService activityService) {
        this.robotRepo = robotRepo;
        this.activityService = activityService;
    }

    // every 3 seconds: update LIVE robot state (FOR EACH ROBOT)
    ...
    public void tickStatus() {
        ...
        for (Robot r : robots) {
            ...
            if (r.getBatteryLevel() == null) r.setBatteryLevel(FULL_BATTERY);
            if (r.getCurrentLocation() == null)
                r.setCurrentLocation(DOCK);
            if (r.getTargetLocation() == null)
                r.setTargetLocation(r.getCurrentLocation());
            if (r.getRobotStatus() == null) r.setRobotStatus(RobotStatus.RESTING);
            ...
            switch (r.getRobotStatus()) {
                ...
                case RESTING -> {
                    ...
                    if (random.nextDouble() < 0.20 && r.getBatteryLevel() > LOW_BATTERY_THRESHOLD + 5) {
                        HouseLocation next = randomHouseLocation();
    ...
    // every 7 seconds: generate an additional “activity feed” item that makes sense (FOR EACH ROBOT)
    ...
    public void tickActivityFeed() {
        ...
        for (Robot r : robots) {
            if (r.getRobotStatus() == null) continue;
    ...
    private HouseLocation randomHouseLocation() {
        HouseLocation[] locations = HouseLocation.values();
